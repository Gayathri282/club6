...
(function() {
    ...
    window.trx_addons_set_storage = function(name, value) {
        ...
        else trx_addons_set_cookie(name, value, 365);
    };
    ...
    window.trx_addons_refresh_list = function(parent_type, parent_val, list_fld, list_lbl, list_not_selected) {
        ...
        function trx_addons_refresh_field_items(data) {
            ...
            for (var i in data) {
                ...
                opt_list += list_type == 'select' ? '<option class="' + data[i]['key'] + '"' + ' value="' + data[i]['key'] + '"' + (data[i]['key'] == list_val ? ' selected="selected"' : '') + '>' + data[i]['value'] + '</option>' : '<label><input type="checkbox"' + ' value="' + data[i]['key'] + '"' + ' name="' + list_name + '"' + '>' + data[i]['value'] + '</label>';
            }
    ...
    window.trx_addons_get_embed_from_url = function(url, autoplay) {
        if (autoplay === undefined) var autoplay = true;
        url = url.replace('/watch?v=', '/embed/').replace('/vimeo.com/', '/player.vimeo.com/video/');
        if (autoplay) {
    ...
    window.trx_addons_is_images_loaded = function(cont) {
        ...
        cont.find('img').each(function() {
            ...
            if (typeof img.naturalWidth == 'number' && typeof img.naturalHeight == 'number') complete = !(this.naturalWidth == 0 && this.naturalHeight == 0);
            else if (typeof img.complete == 'boolean') complete = img.complete;
        });
        return complete;
    };
    ...
    function trx_addons_ready_actions(e, container) {
        ...
        if (TRX_ADDONS_STORAGE['animate_inner_links'] > 0 && !container.hasClass('animate_to_inited')) {
            container.addClass('animate_to_inited').on('click', 'a', function(e) {
                ...
                if (link_parent.parent().hasClass('trx_addons_tabs_titles') || link_obj.hasClass('trx_addons_panel_link') || link_obj.hasClass('trx_addons_popup_link') || link_parent.hasClass('vc_tta-tab') || link_obj.hasClass('vc_pagination-trigger') || link_obj.hasClass('ui-tabs-anchor') || link_parent.hasClass('vc_tta-panel-title') || link_parent.hasClass('wpb_accordion_header') || link_parent.parents('.wc-tabs').length > 0 || link_parent.parents('ul[class*="tabs"]').length > 0) {
                    return true;
                }
        ...
        if (container.find('.trx_addons_video_player.with_cover .video_hover:not(.inited)').length > 0) {
            container.find('.trx_addons_video_player.with_cover .video_hover:not(.inited)').addClass('inited').on('click', function(e) {
                ...
                jQuery(this).parents('.trx_addons_video_player').eq(0).addClass('video_play').find('.video_embed').html(jQuery(this).data('video'));
        ...
        if (TRX_ADDONS_STORAGE['popup_engine'] == 'pretty') {
            container.find('a[href$="jpg"]:not(.inited):not([target="_blank"])' + ',a[href$="jpeg"]:not(.inited):not([target="_blank"])' + ',a[href$="png"]:not(.inited):not([target="_blank"])' + ',a[href$="gif"]:not(.inited):not([target="_blank"])').each(function() {
                if (!jQuery(this).parent().hasClass('woocommerce-product-gallery__image')) {
                    jQuery(this).attr('rel', 'prettyPhoto[slideshow]');
                }
            });
            var images = container.find('a[rel*="prettyPhoto"]:not(.inited)' + ':not(.esgbox)' + ':not(.fancybox)' + ':not([target="_blank"])' + ':not([data-rel*="pretty"])' + ':not([rel*="magnific"])' + ':not([data-rel*="magnific"])' + ':not([data-elementor-open-lightbox="yes"])' + ':not([data-elementor-open-lightbox="default"])').addClass('inited');
            try {
                images.prettyPhoto({
                    social_tools: '',
                    theme: 'facebook',
            ...
            container.find('a[href$="jpg"]:not(.inited):not([target="_blank"])' + ',a[href$="jpeg"]:not(.inited):not([target="_blank"])' + ',a[href$="png"]:not(.inited):not([target="_blank"])' + ',a[href$="gif"]:not(.inited):not([target="_blank"])').each(function() {
                var obj = jQuery(this);
                if (obj.parents('.cq-dagallery').length == 0 && !obj.hasClass('prettyphoto') && !obj.hasClass('esgbox')) {
                    obj.attr('rel', 'magnific');
                }
            });
            var images = container.find('a[rel*="magnific"]:not(.inited)' + ':not(.esgbox)' + ':not(.fancybox)' + ':not([target="_blank"])' + ':not(.prettyphoto)' + ':not([rel*="pretty"])' + ':not([data-rel*="pretty"])' + ':not([data-elementor-open-lightbox="yes"])' + ':not([data-elementor-open-lightbox="default"])').addClass('inited');
            setTimeout(function() {
            ...
            container.find('.sc_layouts_popup:not(.inited),.sc_layouts_panel:not(.inited)').each(function() {
                ...
                link.addClass(is_panel ? 'trx_addons_panel_link' : 'trx_addons_popup_link').data('panel', obj);
                obj.addClass('inited').on('click', '.sc_layouts_panel_close', function(e) {
            ...
            container.find(".trx_addons_panel_link:not(.panel_inited)").addClass('panel_inited').on('click', function(e) {
        ...
        if (container.find('a.post_meta_likes:not(.inited),a.comment_counters_likes:not(.inited)').length > 0) {
            ...
            container.find('a.post_meta_likes:not(.inited),a.comment_counters_likes:not(.inited)').addClass('inited').on('click', function(e) {
                if (!likes_busy) {
                    likes_busy = true;
        ...
        if (container.find('.trx_addons_emotions:not(.inited)').length > 0) {
            ...
            container.find('.trx_addons_emotions:not(.inited)').addClass('inited').on('click', '.trx_addons_emotions_item', function(e) {
                if (!emotions_busy) {
                    emotions_busy = true;
                    var button = jQuery(this);
        ...
        container.find('[data-hover-animation^="animated"]').closest('.elementor-column,.post_layout_custom').each(function() {
            ...
            jQuery(this).addClass('hover-animation-inited').hover(function(e) {
        ...
        cont.find('.sc_column_fixed').each(function() {
            ...
            if (col.attr('class').indexOf('vc_col-lg-') != -1 || col.attr('class').indexOf('vc_col-md-') != -1) {
                ...
                if (col_height < row_height && scroll_offset + top_panel_fixed_height > row_top) {
                    ...
                    if (old_style !== undefined) {
                        col.attr('style', old_style).removeAttr('data-old_style');
                        if (col.prev().hasClass('sc_column_fixed_placeholder')) col.prev().remove();
                    }
                }
            }
        });
        ...
        cont.find('.sc_stack_section_on:not(.elementor-element-edit-mode)').each(function() {
            ...
            if (jQuery(window).width() < 480) {
                if (row.hasClass('sc_stack_section_fixed')) {
                    row.removeClass('sc_stack_section_fixed').css({
                ...
                if (row.hasClass('sc_stack_section_fixed')) {
                    ...
                    if (row_top <= wso) {
                        var row_height = row.outerHeight();
                        row.addClass('sc_stack_section_fixed').data('old-top', row.css('top')).css({
                            top: fixed_rows + 'px'
...
(function() {
    ...
    function trx_addons_sc_fullheight(e, container) {
        ...
        container.find('.trx_addons_stretch_height').each(function() {
            ...
            if (fullheight_row.hasClass('vc_row-o-full-height') || fullheight_row.hasClass('elementor-section-height-full')) {
                ...
                var wh = jQuery(window).height() >= 698 && jQuery(window).width() > 1024 ? jQuery(window).height() - trx_addons_fixed_rows_height() : 'auto';
                if (wh > 0) {
    ...
    jQuery(document).on('action.init_hidden_elements', function() {
        jQuery('.sc_item_filters:not(.inited),.sc_item_pagination:not(.inited)').addClass('inited').on('click', 'a', function(e) {
...
(function() {
    ...
    jQuery(window).on('elementor/frontend/init', function() {
        if (typeof window.elementorFrontend !== 'undefined' && typeof window.elementorFrontend.hooks !== 'undefined') {
            if (elementorFrontend.isEditMode()) {
                ...
                if (body.hasClass('cpt_layouts-template') || body.hasClass('cpt_layouts-template-default') || body.hasClass('single-cpt_layouts')) jQuery('.elementor-element.elementor-widget').addClass('sc_layouts_item');
                jQuery('.elementor-shape').each(function() {
                    var shape = jQuery(this).data('shape');
                    if (shape !== undefined && shape.indexOf('trx_addons_') == 0) trx_addons_load_shape(jQuery(this), shape);
                });
    ...
    window.trx_addons_elementor_animate_items = function(force) {
        jQuery('[class*="animation_type_"]:not(.animation_type_block)' + (!force ? ':not(.animated-separate)' : '')).each(function() {
            ...
            sc.removeClass('elementor-invisible animated ' + trx_addons_elementor_animate_items_animation(item_params) + (sc.data('last-animation') ? ' ' + sc.data('last-animation') : '')).addClass('animated-separate').data('last-animation', trx_addons_elementor_animate_items_animation(item_params));
            if (!cid) {
    ...
    function trx_addons_elementor_animate_items_scroll(force) {
        jQuery('[class*="animated-item"]' + (!force ? ':not(.animated)' : '')).each(function(idx) {
            ...
            if (item.offset().top + 50 < jQuery(window).scrollTop() + jQuery(window).height() && (force || !item.hasClass('animated'))) {
                ...
                if (force) {
                    item.removeClass(item_animation + (item.data('last-animation') ? ' ' + item.data('last-animation') : '')).addClass('elementor-invisible').data('last-animation', item_animation);
                } else {
    ...
    function trx_addons_elementor_animate_items_delay(params, sc, idx) {
        return sc.hasClass('animation_type_sequental') ? (params && params._animation_delay ? params._animation_delay : 150) * idx : trx_addons_random(0, params && params._animation_delay ? params._animation_delay : 1500);
    }
...
(function() {
    ...
    window.trx_addons_parallax = function($target, init_type) {
        ...
        self.create_layers = function() {
            ...
            jQuery.each(settings, function(index, block) {
                ...
                if ('' !== image || 'none' !== type) {
                    ...
                    $layout = jQuery('<div class="sc_parallax_block sc_parallax_block_type_' + type + (is_safari ? ' is-safari' : '') + ('MacIntel' == platform ? ' is-mac' : '') + (typeof block['class'] !== undefined && block['class'] != '' ? ' ' + block['class'] : '') + '"><div class="sc_parallax_block_image"></div></div>').prependTo($target).css(layout_init);
                    layout_init = {
        ...
        self.mouse_move_handler = function(e) {
            ...
            wst = $window.scrollTop(), ww = $window.width();
            wh = $window.height();
            var cx = Math.ceil(tw / 2),
                cy = Math.ceil(th / 2),
                dx = e.clientX - tl - cx,
            ...
            jQuery.each(scroll_list, function(index, block) {
                if (parallax_type == 'layers') {
                    ...
                    if (w_top <= obj_bottom && obj_top <= w_bottom - bottom_delta && !entrance_complete) {
                        ...
                        var delta = entrance ? 1 : Math.max(1, (wh + obj_height) * params.amplitude / 100),
                        ...
                        var scroller_init = {
                            ...
                            val = 0;
                        if (step_opacity != 0) {
                            scroller_init.opacity = trx_addons_round_number(start ? Math.min(1, 1 - shift * step_opacity + params.opacity) : 1 + shift * step_opacity, 2);
                        }
                        if (step_x != 0) {
                            val = Math.round(start ? params.x - shift * step_x : shift * step_x - (params.type == 'bg' && params.x > 0 ? params.x : 0));
                            if (start && ((params.x < 0 && val > 0) || (params.x > 0 && val < 0))) val = 0;
                            transform += 'translateX(' + val + 'px)';
                            scroller_init.x = val;
                        }
                        if (step_y != 0) {
                            val = Math.round(start ? params.y - shift * step_y : shift * step_y - (params.type == 'bg' && params.y > 0 ? params.y : 0));
                            if (start && ((params.y < 0 && val > 0) || (params.y > 0 && val < 0))) val = 0;
                            transform += (transform != '' ? ' ' : '') + 'translateY(' + val + 'px)';
                            scroller_init.y = val;
                        }
                        if (step_rotate != 0) {
                            val = trx_addons_round_number(start ? params.rotate - shift * step_rotate : shift * step_rotate, 2);
                            if (start && ((params.rotate < 0 && val > 0) || (params.rotate > 0 && val < 0))) val = 0;
                            transform += (transform != '' ? ' ' : '') + 'rotate(' + val + 'deg)';
                            scroller_init.rotation = val;
                        }
                        if (step_scale != 0) {
                            val = trx_addons_round_number(start ? 1 + params.scale / 100 - shift * step_scale : 1 + shift * step_scale - (params.type == 'bg' && params.scale < 0 ? params.scale / 100 : 0), 2);
                        ...
                        if (['chars', 'words'].indexOf(params.text) != -1 && obj.data('element_type') !== undefined) {
                            ...
                            if (inner_obj.length == 0) {
                                inner_obj = obj.find(sc == 'trx_sc_title' ? '.sc_item_title_text,.sc_item_subtitle' : (sc == 'trx_sc_supertitle' ? '.sc_supertitle_text' : (sc == 'heading' ? '.elementor-heading-title' : 'p')));
                                if (inner_obj.length > 0) {
                                    inner_obj.each(function(idx) {
                                        inner_obj.eq(idx).html(params.text == 'chars' ? self.wrap_chars(inner_obj.eq(idx).html()) : self.wrap_words(inner_obj.eq(idx).html()));
                                    });
        ...
        self.wrap_chars = function(txt) {
            ...
            for (var i = 0; i < txt.length; i++) {
                ...
                rez += in_tag ? ch : '<span class="sc_parallax_text_block">' + (txt.substr(i, 1) == ' ' ? '&nbsp;' : txt.substr(i, 1)) + '</span>';
                if (ch == '>') {
                    in_tag = false;
                }
            }
    ...
    jQuery('.mc4wp-form-fields').each(function() {
        ...
        if (count_mc4wp > 1) {
            jQuery(this).find('input[type="checkbox"]').each(function() {
                var id = jQuery(this).attr('id');
                jQuery(this).attr('id', id + '_' + count_mc4wp).next('label').attr('for', id + '_' + count_mc4wp);
            });
        }
    });
});
...
jQuery(document).on('action.init_hidden_elements', function(e, container) {
    ...
    function trx_addons_detect_active_toc(force) {
        ...
        toc_menu_items.each(function(idx) {
            ...
            var off_next = id_next && idx < toc_menu_items.length - 1 && jQuery(id_next).length > 0 ? parseInt(jQuery(id_next).offset().top, 10) : 1000000;
...
(function() {
            ...
            gmap.each(function() {
                ...
                map.find('.sc_googlemap_marker').each(function() {
                    ...
                    map_markers.push({
                        ...
                        animation: marker.data('animation') == 'drop' ? google.maps.Animation.DROP : (marker.data('animation') == 'bounce' ? google.maps.Animation.BOUNCE : false),
                    });
                });
    ...
    function trx_addons_sc_googlemap_create(map, coords) {
        ...
        googlemap.maps[id] = {
            ...
            opt: {
                ...
                zoom: coords.zoom ? parseInt(coords.zoom, 10) : (coords.markers.length == 1 && center === null ? 16 : 0),
                zoomControl: true,
                panControl: true,
    ...
    function trx_addons_sc_googlemap_build(id) {
        ...
        jQuery(document).on('action.resize_trx_addons', function() {
            if (googlemap.maps[id].map && jQuery('#' + id).parent().hasClass('ready')) {
                if (googlemap.maps[id].opt['center']) {
    ...
    function trx_addons_sc_googlemap_add_markers(id) {
        for (var i = 0; i < googlemap.maps[id].markers.length; i++) {
            ...
            if (!googlemap.maps[id].markers[i].latlng) {
                ...
                if (googlemap.maps[id].opt['center'] == null && (googlemap.maps[id].markers.length == 1 || googlemap.maps[id].opt['zoom'] > 0)) {
                    ...
                    google.maps.event.addListener(googlemap.maps[id].markers[i].marker, "click", function(e) {
                        ...
                        for (var j = 0; j < googlemap.maps[id].markers.length; j++) {
                            if (trx_addons_googlemap_compare_latlng(latlng, googlemap.maps[id].markers[j].latlng)) {
                                ...
                                googlemap.maps[id].markers[j].infowindow.open(googlemap.maps[id].map, googlemap.maps[id].markers[j].marker);
                                break;
                            }
                        }
                    });
        ...
        if (googlemap.maps[id].markers_inited == googlemap.maps[id].markers.length) {
            if (googlemap.maps[id].markers_inited > 1) {
                var markers = [];
...
(function() {
    ...
    function trx_addons_init_audio() {
        jQuery('.trx_addons_audio_wrap:not(.inited)').addClass('inited').each(function() {
            ...
            if (audio_wrap.hasClass('list')) {
                ...
                audio_wrap.find('audio').each(function() {
                    var id = jQuery(this).attr('id');
    ...
    function trx_addons_audio_height() {
        ...
        jQuery('.trx_addons_audio_wrap').each(function() {
            jQuery(this).removeClass('resized');
            if (jQuery(this).hasClass('list')) {
                var height = 0;
                jQuery(this).find('.trx_addons_audio_player').each(function() {
...
(function() {
        ...
        if (container.find('.sc_slider_controller:not(.inited)').length > 0) {
            container.find('.sc_slider_controller:not(.inited)').each(function() {
                ...
                jQuery('#' + slider_id + ' .swiper-slide').each(function(idx) {
                    ...
                    var image = controller_style.indexOf('thumbs') == -1 ? '' : (slide.data('image') ? slide.data('image') : (slide.find('img').length > 0 ? slide.find('img').eq(0).attr('src') : (slide.css('background-image') != '' ? slide.css('background-image').replace('url(', '').replace(')', '').replace(';', '') : '')));
                    var title = controller_style.indexOf('titles') == -1 ? '' : (slide.data('title') ? slide.data('title') : slide.find('[class*="_item_title"]').text());
                    var cats = slide.data('cats');
                    var date = slide.data('date');
                    controller_html += '<div class="slider-slide swiper-slide"' + ' style="' + (image !== undefined && image !== '' ? 'background-image: url(' + image + ');' : '') + '"' + '>' + '<div class="sc_slider_controller_info">' + '<span class="sc_slider_controller_info_number">' + (idx < 9 ? '0' : '') + (idx + 1) + '</span>' + '<span class="sc_slider_controller_info_title">' + (title ? title : 'Slide ' + (idx + 1)) + '</span>' + '</div>' + '</div>';
                });
            ...
            container.find('.sc_slider_controls:not(.inited)').each(function() {
                ...
                if (pagination_style == 'thumbs') {
                    ...
                    slides.each(function(idx) {
                        ...
                        html += '<span class="slider_pagination_button_wrap swiper-pagination-button-wrap" style="width: ' + Math.round(100 / total, 2) + '%;">' + '<span class="slider_pagination_button swiper-pagination-button"' + (image !== undefined ? ' style="background-image: url(' + image + ');"' : '') + '></span>' + '</span>';
                    });
                    controls.find('.slider_pagination_wrap').html(html);
                } else if (pagination_style == 'fraction') {
                    controls.find('.slider_pagination_wrap').html('<span class="slider_pagination_current swiper-pagination-current">1</span>' + '/' + '<span class="slider_pagination_total swiper-pagination-total">' + total + '</span>');
        ...
        if (container.find('.slider_swiper:not(.inited)').length > 0) {
            container.find('.slider_swiper:not(.inited)').each(function() {
                ...
                TRX_ADDONS_STORAGE['swipers'][id] = new Swiper('.' + id, {
                    ...
                    initialSlide: 0,
                    speed: 600,
                    loop: slider.data('slides-overflow') != 1,
                    loopedSlides: spv,
                    slidesPerView: spv,
                    spaceBetween: space,
                    ...
                    autoplay: slider.hasClass('slider_noautoplay') || (typeof slider.data('autoplay') !== 'undefined' && slider.data('autoplay') == 0) || interval == 0 ? false : {
    ...
    function trx_addons_resize_sliders(e, container) {
        ...
        container.find('.slider_container.inited').each(function() {
            ...
            if (direction != 'vertical') direction = 'horizontal';
            var on_resize = false;
            var max_width = slider.closest('.elementor-widget-wrap,.vc_column-inner,.content,.page_wrap,body').width();
...
(function() {
            ...
            jQuery(selector).show().each(function() {
                ...
                function trx_addons_before_show_menu(menu_item) {
                    if (menu_item.parents("ul").length > 1) {
                        ...
                        if (par_offset + par_width + ul_width > w - 10 && par_offset - ul_width > 0) menu_item.addClass('submenu_left');
                        else menu_item.removeClass('submenu_left');
                    }
    ...
    function trx_addons_menu_collapse() {
        ...
        jQuery('.sc_layouts_menu:not(.sc_layouts_menu_dir_vertical)').each(function() {
            ...
            if (!sc_layouts_item.parent().hasClass('wpb_wrapper') && !sc_layouts_item.parent().hasClass('sc_layouts_column') && !sc_layouts_item.parent().hasClass('elementor-widget-wrap')) return;
            ...
            sc_layouts_item.parent().find('>div').each(function() {
                ...
                if (jQuery(this).hasClass('vc_empty_space') || jQuery(this).hasClass('vc_separator') || jQuery(this).hasClass('elementor-widget-spacer') || jQuery(this).hasClass('elementor-widget-divider')) {
                    if (in_group == 1) in_group = 2;
                    else w_siblings = 0;
                } else {
                    if (jQuery(this).find('#' + ul_id).length > 0) in_group = 1;
                    else w_siblings += (jQuery(this).outerWidth() + Math.ceil(parseFloat(jQuery(this).css('marginLeft'))) + Math.ceil(parseFloat(jQuery(this).css('marginRight'))));
                }
            });
            w_max -= w_siblings;
            var w_all = 0;
...
jQuery(document).on('action.ready_trx_addons', function() {
    ...
    jQuery('.trx_addons_woocommerce_search_form:not(.inited)').each(function() {
        jQuery(this).addClass('inited').on('submit', function(e) {
            ...
            var params = trx_addons_woocommerce_search_form_get_params(form);
            if (params === false) button.attr('disable', 'disable');
            else button.removeAttr('disable');
